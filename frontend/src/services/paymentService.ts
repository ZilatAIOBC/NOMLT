...
export async function createCheckoutSession(params: {
  ...
  const res = await authHelper.authFetch(
    `${apiBaseUrl}/api/payments/create-checkout-session`,
    {
      method: "POST",
      credentials: "include",
...
export async function retrieveCheckoutSession(sessionId: string) {
  const res = await authHelper.authFetch(
    `${apiBaseUrl}/api/payments/checkout-session?session_id=${encodeURIComponent(
      sessionId
    )}`,
    {
      method: "GET",
...
export async function createTopupSession(params: {
  ...
  const res = await authHelper.authFetch(
    `${apiBaseUrl}/api/payments/topup/session`,
    {
      method: "POST",
      credentials: "include",
      body: JSON.stringify(params),
  ...
  if (!res.ok) {
    const msg = await res.text();
    console.warn("[frontend][topup] topup/session failed", {
      status: res.status,
      msg,
    });
