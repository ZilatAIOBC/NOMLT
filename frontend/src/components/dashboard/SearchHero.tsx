...
const SearchHero: React.FC<Props> = ({ onGenerate }) => {
  ...
  const handleGenerate = () => {
    if (prompt.trim()) {
      ...
      navigate(
        `/dashboard/text-to-image?prompt=${encodeURIComponent(prompt.trim())}`
      );
    }
  };
      ...
      <div className="rounded-2xl p-4 sm:p-8 lg:p-16 bg-gradient-to-r from-[#0D131E] to-[#7D3CEA] w-full">
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-white text-sm sm:text-base lg:text-lg">
            Transform your words into visual masterpieces: Leverage AI
            technology to craft breathtaking visuals.
          </p>
        </div>
