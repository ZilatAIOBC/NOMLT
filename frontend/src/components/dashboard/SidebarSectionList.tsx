...
const SidebarSectionList: React.FC<Props> = ({
  ...
  const ItemIcon: React.FC<{ item: SidebarItem; active: boolean }> = ({
    item,
    active,
  }) => {
    ...
    if (isCustomIcon) {
      return (
        <img
          ...
          } ${sizeClass} transition-opacity hover:opacity-80`}
        />
      );
    }
  ...
  return (
    <>
      {sections.map((section) => (
        <div
          key={section.title}
          className="space-y-2 lg:space-y-1 xl:space-y-2"
        >
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex ${
                    ...
                    isCollapsible && !isExpanded ? "px-3 py-3" : "px-3 py-3"
