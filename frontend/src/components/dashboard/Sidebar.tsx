...
import { LogOut, ChevronLeft, ChevronRight, ShieldOff } from "lucide-react";
import { authService } from "../../services/authService";
import SidebarSectionList, {
  SidebarItem as TSidebarItem,
  SidebarSection as TSidebarSection,
} from "./SidebarSectionList";
        ...
        items: [
          {
            ...
            href: "/dashboard/view-generations",
            icon: "viewgenerations",
  ...
  const renderHomeItem = useCallback(() => {
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) =>
          `flex ${
            isMobile && isExpanded ? "w-[199px]" : ""
          } items-center gap-3 px-3 py-3 rounded-lg text-sm font-medium ${
            isActive
              ...
              {isCustomIcon ? (
                ...
                (() => {
                  ...
                  return (
                    <IconComp
                      className={`${sizeClass} ${
                        isActive ? "text-white" : homeItem.color
                      }`}
                    />
                  );
                })()
      ...
      >
        ...
        <div className="p-6 border-b border-white/10">
          <Link
            ...
            className={`flex items-center ${
              isCollapsible && !isExpanded ? "justify-start" : "justify-start"
            }`}
          >
            <img
              src={
                isCollapsible && !isExpanded
                  ? "/videogenerations.svg"
                  : "/logo.svg"
              }
              alt="NOLMT.AI"
              className={`${
                isCollapsible && !isExpanded ? "h-12 w-12" : "h-10"
              }`}
              style={{
                width: isCollapsible && !isExpanded ? "48px" : "auto",
                height: isCollapsible && !isExpanded ? "48px" : "40px",
              }}
            />
          </Link>
        </div>
        ...
        <div className="border-t border-white/10 space-y-2">
          ...
          >
            <span
              className={`inline-flex ${
                isCollapsible && !isExpanded ? "hover:opacity-80" : ""
              } transition-opacity`}
            >
              <LogOut
                className={`${
                  (isCollapsible && !isExpanded) || isMobile || isTablet
                    ? "w-6 h-6"
                    : "w-5 h-5"
