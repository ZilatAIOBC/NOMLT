  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (result.data.outputs && result.data.outputs.length > 0) {
        ...
        // Fetch and log usage summary for<｜tool▁sep｜>current user (image-to-image key will be included)
        try {
          const summary = await fetchUsageSummary();
          console.log('Usage summary after image-to-image generation:', summary);
        } catch (e) {
