...
import {
  ...
  uploadImageToUrl,
  ImageToImageRequest,
} from "../../services/imageToImageService";
...
const ImageToImage: React.FC = () => {
  ...
  const computeNormalizedSize = (w: number, h: number) => {
    ...
    while (newW * newH < MIN_TOTAL_PIXELS && attempts < 10) {
      if (newW >= newH && newW + STEP <= MAX_SIDE) newW += STEP;
      else if (newH + STEP <= MAX_SIDE) newH += STEP;
      else break;
      attempts++;
    }
          ...
          <div className="w-full xl:max-w-md xl:mx-0 space-y-6">
            ...
            <div className="border border-[#25202D] rounded-lg p-4 space-y-6 h-[60vh] xl:h-auto overflow-y-auto xl:overflow-visible">
              ...
              <div>
                ...
                <IdeaChips
                  ideas={[
                    "Replace the stick with a flame",
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setWidth(
                          Math.max(
                            256,
                            Math.min(3183, Number(e.target.value) || 0)
                          )
                        )
                      }
                    />
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    onClick={() => {
                      if (generatedImage) {
                        navigate(
                          `/dashboard/image-to-video?imageUrl=${encodeURIComponent(
                            generatedImage
                          )}`
                        );
                      }
