...
import {
  ...
  TextToVideoRequest,
} from "../../services/textToVideoService";
...
const TextToVideo: React.FC = () => {
  ...
  const [resolution, setResolution] = useState<"832x480" | "480x832">(
    "832x480"
  );
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (audioFile) {
        ...
        const { uploadImageToUrl } = await import(
          "../../services/imageToVideoService"
        );
      ...
      if (
        ...
        setCensoredModal({
          isOpen: true,
          nsfwWord: respData.nsfwWord || null,
          planName: respData.planName || null,
          reason: respData.reason || null,
          unfilteredModeUrl:
            respData.unfilteredModeUrl || "/dashboard/uncensored",
        });
        setError("");
      } else {
        setError(
            ...
            <div className="border border-[#25202D] rounded-lg p-4 space-y-6 h-[60vh] xl:h-auto overflow-y-auto xl:overflow-visible">
              ...
              <div>
                ...
                <textarea
                  ...
                  style={
                    { "--tw-ring-color": "#8A3FFC" } as React.CSSProperties
                  }
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {resolutionOptions.map((option) => {
                    const isSelected =
                      resolution === (option.value as "832x480" | "480x832");
                    return (
                      <button
                        key={option.value}
                        onClick={() =>
                          setResolution(option.value as "832x480" | "480x832")
                        }
              ...
              <div className="bg-[#0D131F] border border-gray-700 rounded-lg p-4">
                <div className="text-sm text-white space-y-1">
                  ...
                  <p>
                    ...
                    credits you can run this model approximately{" "}
                    <span
                      className="font-semibold"
                      style={{ color: "#8A3FFC" }}
                    >
                      {currentCredits != null && CREDIT_COST > 0
                        ? Math.floor(
