...
import AddonPricing from "../../components/admin/AddonPricing";
...
export default function PlansAndPricing() {
  const [activeTab, setActiveTab] = useState<"plans" | "credits" | "addons">(
    "plans"
  );
    ...
    setPlans((prev) =>
      prev.map((p) => {
        ...
        return {
          ...p,
          name:
            payload.display_name !== undefined ? payload.display_name : p.name,
          price:
            payload.price_monthly !== undefined
              ? payload.price_monthly
              : result.price_monthly ?? p.price,
          credits:
            payload.credits_included !== undefined
              ? payload.credits_included
              : result.credits_included ?? p.credits,
          featuresList:
            payload.features !== undefined ? payload.features : p.featuresList,
        };
      })
    );
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-400">
              Manage subscription plans and credit pricing
            </p>
          </div>
