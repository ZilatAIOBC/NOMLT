...
} = require("../services/imageToImageService");
...
router.post("/", auth, checkCredits('image_to_image'), async (req, res) => {
  try {
    ...
    if (client && userId) {
      try {
        // Find any active image_to_image model id
        const { data: models } = await client
          .from('ai_models')
          .select('id')
          .eq('category', 'image_to_image')
          .eq('is_active', true)
          .limit(1);
        ...
        client
          ...
          .insert({
            user_id: userId,
            model_id: modelId,
            endpoint: 'image-to-image:create',
            api_cost: 0,
...
// GET /api/image-to-image/result?url=ENCODED_URL
router.get("/result", auth, async (req, res) => {
  try {
    ...
    const result = await getImageToImageResult(
      decodedUrl,
    ...
    const generation = await createGeneration({
      ...
      settings: {
        ...
        size: req.body.size,
        enable_base64_output: req.body.enable_base64_output,
        enable_sync_mode: req.body.enable_sync_mode,
      },
