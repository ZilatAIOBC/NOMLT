...
// Add credits to a user's balance (delta-based, no RPC)
router.post('/:id/credits', auth, requireAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message: 'Invalid "delta". It must be a non-negative integer.',
      });
    }
    ...
    // 1) Read current credits row (if any)
    ...
    // If an unexpected read error occurred
    if (readErr && readErr.code !== 'PGRST116') {
      return res.status(500).json({
        success: false,
        message: 'Failed to read current credits',
        error: readErr.message,
      });
    }

    // 2) No row yet: insert initial balance = delta
    if (!existing) {
      ...
      return res.json({
        ...
        data: { id, credits: inserted.balance },
      });
    }

    // 3) Row exists: add delta to balance & lifetime_earned
    const newBalance = (existing.balance || 0) + delta;
    const newLifetimeEarned = (existing.lifetime_earned || 0) + delta;
    ...
    return res.json({
      success: true,
      message: 'Credits added successfully',
      data: { id, credits: updated.balance },
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
